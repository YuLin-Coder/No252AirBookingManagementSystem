...
var userId = <%=userId%>;
	...
	function deleteSjshaochu(){
		...
		var ids=strIds.join(",");
		$.messager.confirm("系统提示","您确认要取消这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../deleteSjshaochu",{delIds:ids},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功取消<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
	...
	function shenheSjshaochu(){
		...
		$.messager.confirm("系统提示","您确认要执行吗？",function(r){
			if(r){
				$.post(url,{sjshaochuType2:1},function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						$("#dg").datagrid("reload");
					}else{
...
<!--startprint-->
	<table id="dg" title="<%=newJcpeizhi.getShujuBieming()%><%=newJcpeizhi.getSjshaochuBieming()%>" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getSjshaochus?sjshaochuType2=0&userId=<%=userId %>" fit="true" rownumbers="true" toolbar="#tb">
		<thead>
			<tr>
	...
	<div id="tb">
		<div>
			<a href="javascript:shenheSjshaochu()" class="easyui-linkbutton" iconCls="icon-add" plain="true">付款</a>
			<a href="javascript:deleteSjshaochu()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">取消</a>
		</div>
		<div>
