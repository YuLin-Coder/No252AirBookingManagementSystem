...
<title><%=newJcpeizhi.getGonggaoBieming()%>信息</title>
...
var url;
	...
	function openGonggaoAddDialog(){
		$("#dlg").dialog("open").dialog("setTitle","添加<%=newJcpeizhi.getGonggaoBieming()%>信息");
		url="../addGonggao?ggtypeId=1";
	}
	...
	function openGonggaoModifyDialog(){
		...
		$("#dlg").dialog("open").dialog("setTitle","编辑<%=newJcpeizhi.getGonggaoBieming()%>信息");
		$("#fm").form("load",row);
	...
	function shangchuanGonggao(){
		...
		var row=selectedRows[0];
		$("#shangchuan").dialog("open").dialog("setTitle","上传<%=newJcpeizhi.getGonggaoBieming()%>信息");
...
<!--startprint-->
	<table id="dg" title="<%=newJcpeizhi.getGonggaoBieming()%>信息" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getGonggaos?ggtypeId=1" fit="true" rownumbers="true" toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="gonggaoId" width="10" formatter="chakanNeirong">查看</th>
				<th field="gonggaoName" width="80">标题</th>
				<th field="gonggaoDate" width="20" formatter="datetostr">时间</th>
			</tr>
		</thead>
	</table>
	...
	<div id="dlg" class="easyui-dialog" style="width: 850px;height: 380px;padding: 10px 20px"
		...
		<form name="fm" id="fm" method="post">
			<table>
				<tr>
					<td>名称：</td>
					<td><input style="width:400px"  type="text" name="gonggaoName" id="gonggaoName" class="easyui-validatebox" required="true"/></td>
				</tr>
				<tr>
